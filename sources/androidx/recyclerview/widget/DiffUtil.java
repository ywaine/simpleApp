...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f71x - o2.f71x;
            return cmpX == 0 ? o1.f72y - o2.f72y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f71x += range.oldListStart;
                snake.f72y += range.newListStart;
                Range left = arrayList.isEmpty() ? new Range() : (Range) arrayList.remove(arrayList.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f71x;
                    left.newListEnd = snake.f72y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f71x - 1;
                    left.newListEnd = snake.f72y;
                } else {
                    left.oldListEnd = snake.f71x;
                    left.newListEnd = snake.f72y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f71x + snake.size;
                    right.newListStart = snake.f72y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f71x + snake.size + 1;
                    right.newListStart = snake.f72y + snake.size;
                } else {
                    right.oldListStart = snake.f71x + snake.size;
                    right.newListStart = snake.f72y + snake.size + 1;
                }
                stack.add(right);
            } else {
                arrayList.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k <= d) {
                    ...
                    if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || iArr[kOffset + k] < iArr2[kOffset + k]) {
                        ...
                        outSnake.f71x = iArr2[kOffset + k];
                        outSnake.f72y = outSnake.f71x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f71x = iArr2[kOffset + backwardK];
                        outSnake2.f72y = outSnake2.f71x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f71x != 0 || firstSnake.f72y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f71x + snake.size;
                int endY = snake.f72y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f71x + j;
                    int newItemPos = snake.f72y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f71x + snakeSize2;
                int endY2 = snake.f72y + snakeSize2;
                if (endX < posOld) {
                    endY = endY2;
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f71x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f71x + i2, 1, this.mCallback.getChangePayload(snake.f71x + i2, snake.f72y + i2));
                    }
                }
                posOld = snake.f71x;
                posNew = snake.f72y;
